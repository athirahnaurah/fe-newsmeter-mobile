...
const DetailBeritaByKategori = ({navigation}) => {
  // const tagsStyles = {
  //   a: {
  ...
  const {newsbykategori} = useSelector(state => state.newsReducer);
  const {width : contentWidth} = ms.width((windowWidth * 90) / 100);
  const source = {
    html: newsbykategori?.content
  }
  ...
  const OpenURLButton = ({url, node}) => {
    ...
    return <TouchableOpacity onPress={handlePress}><Text style={[ms.fzBC(12, '400', colors.blue), ms.width((windowWidth * 75) / 100),]}> {newsbykategori?.original}</Text></TouchableOpacity>;
  };
      ...
      <ScrollView>
        ...
        <View
          ...
          <View style={[ms.mgB(20)]}>
            <Image
              source={{uri : newsbykategori?.image}}
              style={[
                ms.width((windowWidth * 90) / 100),
        ...
        <View
          ...
          <OpenURLButton url={newsbykategori?.original}>link</OpenURLButton>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
...
export default DetailBeritaByKategori;
