...
const DetailBeritaByMedia = ({navigation}) => {
  // const tagsStyles = {
  //   a: {
  ...
  const {newsbymedia} = useSelector(state => state.newsReducer);
  const {width : contentWidth} = ms.width((windowWidth * 90) / 100);
  const source = {
    html: newsbymedia?.content
  }
  ...
  const OpenURLButton = ({url, node}) => {
    ...
    return <TouchableOpacity onPress={handlePress}><Text style={[ms.fzBC(12, '400', colors.blue), ms.width((windowWidth * 75) / 100),]}> {newsbymedia?.original}</Text></TouchableOpacity>;
  };
    ...
    <SafeAreaView style={[ms.containerPage]}>
      <ScrollView>
        ...
        <View
          ...
          <OpenURLButton url={newsbymedia?.original}>link</OpenURLButton>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
...
export default DetailBeritaByMedia;
